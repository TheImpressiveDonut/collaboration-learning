...
class Client(object):
        ...
        for batch_idx, (data, target) in enumerate(self.train_loader):
            ...
            if soft_decision_target is None:
                ...
                soft_decision_loss = torch.mean(
                    -torch.sum(soft_decision_target[ref_idx, :] * torch.log(soft_decision + 1e-8), 1))
    ...
    @torch.no_grad()
    def calculate_trust_weight_mat(self, soft_decision_list: List[Tensor]) -> float:
    ...
    @torch.no_grad()
    def calculate_soft_decision_target(self, soft_decision_list: List[Tensor], global_round: int
                                       ) -> Tuple[Tensor, Tensor]:
        match self.consensus:
            case ConsensusName.soft_assignment:
                match self.trust:
                    case TrustName.static:
                        if not self.trust_weights:
                            self.trust_weights = self.calculate_trust_weight_mat(soft_decision_list)
                ...
                weighted_soft_decisions = [self.trust_weights[i] * soft_decision_list[i] for i in
