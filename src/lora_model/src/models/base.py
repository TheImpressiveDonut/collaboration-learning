...
class GPTBase(nn.Module):
    ...
    def get_num_params(self, non_embedding=True, only_trainable=False):
        """
        ...
        """
        n_params = sum(p.numel() if not only_trainable or p.requires_grad else 0 for p in self.parameters())
        if non_embedding:
