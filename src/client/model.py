...
def lora_model(model: nn.Module, lora_freeze_all_non_lora: bool) -> None:
    if lora_freeze_all_non_lora:
        ...
        for _, module in model.named_modules():
            if isinstance(module, LoRALinear) or isinstance(module, LoRAEmbedding):
...
class LoRALinear(nn.Linear):
    ...
    def __init__(self, in_features: int, out_features: int,
        ...
        if lora_rank > 0:
            ...
            self.lora_A = nn.Parameter(torch.empty((in_features, lora_rank), device=self.weight.device))
            self.lora_B = nn.Parameter(torch.empty((lora_rank, out_features), device=self.weight.device))
    ...
    def forward(self, input: Tensor) -> Tensor:
        ...
        if not self.lora_merged and self.lora_rank > 0:
            x += self.lora_dropout(input) @ self.lora_A @ self.lora_B * self.lora_scaling
        return x
...
class LoRAEmbedding(nn.Embedding):
    def __init__(self, num_embeddings: int, embedding_dim: int,
                 lora_rank: int, lora_alpha: float) -> None:
        ...
        if lora_rank > 0:
            self.lora_alpha = lora_alpha
            self.lora_scaling = self.lora_alpha / self.lora_rank
            self.lora_A = nn.Parameter(torch.empty((lora_rank, num_embeddings), device=self.weight.device))
            self.lora_B = nn.Parameter(torch.empty((embedding_dim, lora_rank), device=self.weight.device))
    ...
    def forward(self, input: torch.Tensor):
        x = super().forward(input)
        if not self.lora_merged and self.lora_rank > 0:
            embedding_A = torch.embedding(self.lora_A.T, input)
            x += embedding_A @ self.lora_B.T * self.lora_scaling
        return x
    ...
    def train(self: T, mode: bool = True) -> T:
        ...
        if mode:
            if self.lora_merged and self.lora_rank > 0:
                self.weight.data -= (self.lora_B @ self.lora_A).T * self.lora_scaling
                self.lora_merged = False
        else:
            if not self.lora_merged and self.lora_rank > 0:
                self.weight.data += (self.lora_B @ self.lora_A).T * self.lora_scaling
...
class GPTLoRA(nn.Module):
    ...
    def get_num_params(self, non_embedding=True, lora_params=False, only_trainable=False):
        ...
        if not lora_params:
            n_params = sum(
                p.numel() if not 'lora' in n and not (only_trainable and not p.requires_grad) else 0 for n, p in
            ...
            for name in named_params.copy():
                if 'lora_A' in name:
                    x = name.replace('lora_A', 'weight')
                    y = name.replace('lora_A', 'bias')
                    if x in named_params:
                        named_params.remove(x)
                    if y in named_params:
                        named_params.remove(y)

            n_params = sum(
                p.numel() if n in named_params and not (only_trainable and not p.requires_grad) else 0 for n, p in
                self.named_parameters())
    ...
    def from_pretrained(cls, model_type: str, override_args: Namespace = None, verbose: bool = False):
        ...
        config_args['lora_embedding'] = override_args.lora_embedding
        ...
        for mn, m in self.named_modules():
            for pn, p in m.named_parameters():
                ...
                if pn.endswith('bias'):
                    ...
                    no_decay.add(fpn)
                elif (pn.endswith('weight') or pn.endswith('lora_A') or pn.endswith('lora_B')) and isinstance(m,
