...
class LambdaLayer(nn.Module):
    def __init__(self, lambd: Callable[[Tensor], Tensor]) -> None:
...
class ResNet(nn.Module):
    def __init__(self, block: nn.Module, num_blocks: List[int], num_classes: int = 10) -> None:
...
def resnet32(num_classes: int) -> ResNet:
    return ResNet(BasicBlock, [5, 5, 5], num_classes)


def resnet44(num_classes: int) -> ResNet:
...
def resnet110(num_classes: int) -> ResNet:
    return ResNet(BasicBlock, [18, 18, 18], num_classes)


def resnet1202(num_classes: int) -> ResNet:
