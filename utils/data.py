...
from utils.exceptions import UnknownPartitionArgumentException, UnknownSplitDataModeException
...
def __get_pat_partition(dataset_label: np.ndarray, config: Config) -> List[np.ndarray]:
    ...
    class_num_per_client = np.full(config.num_clients, config.class_per_client, dtype=int)

    for i in range(config.num_classes):
        selected_clients = np.argwhere(class_num_per_client > 0).flatten()
        selected_clients = selected_clients[:int(config.num_clients / config.num_classes * config.class_per_client)]
        ...
        if config.balance:  # @todo shuffle indices ?
            idxs = np.array_split(idx_for_each_class[i], num_sample_per_clients)
        else:
            num_samples = np.random.randint(
                int(max(num_sample_per_clients / 10, least_samples / config.num_classes)),
...
def __get_dir_partition(dataset_label: np.ndarray, config: Config) -> List[np.ndarray]:
    data_idx_map = [np.array([], dtype=int) for _ in range(config.num_clients)]
    least_samples = config.batch_size / (1 - config.train_size)
    ...
    while min_size < least_samples:
        idx_batch = [[] for _ in range(config.num_clients)]
        for k in range(K):
            idx_k = np.where(dataset_label == k)[0]
            np.random.shuffle(idx_k)
            proportions = np.random.dirichlet(np.repeat(config.alpha, config.num_clients))
            proportions = np.array(
                [p * (len(idx_j) < N / config.num_clients) for p, idx_j in zip(proportions, idx_batch)])
...
def clients_split(data: Data, config: Config) -> Tuple[ClientsData, ClientsDataStatistics]:
    clients_data, statistic = [], []
    dataset_image, dataset_label = data

    match config.partition:
        case 'pat':
            data_idx_map = __get_pat_partition(dataset_label, config)
        case 'dir':
            data_idx_map = __get_dir_partition(dataset_label, config)
        case _:
            raise UnknownPartitionArgumentException(config.partition)
...
def train_test_ref_split(clients_data: ClientsData, config: Config, mode: str = 'by-ratio'
    ...
    for i in range(len(clients_data)):
        X_train, X_test, y_train, y_test = train_test_split(
            clients_data[i][0], clients_data[i][1], train_size=config.train_size, shuffle=True)
